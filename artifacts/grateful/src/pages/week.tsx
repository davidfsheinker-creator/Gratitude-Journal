...
export function Week() {
  ...
  const { data: summary, isLoading } = useGetWeeklySummary(startDate, {
    query: {
      queryKey: getGetWeeklySummaryQueryKey(startDate),
      enabled: !!startDate,
    },
  });
      ...
      {isLoading ? (
        ...
        >
          ...
          <p className="text-muted-foreground text-sm">
            {isCurrentWeek
              ? "Start writing today — your week's story is still being written."
          ...
          {(summary.moodBreakdown.great + summary.moodBreakdown.okay + summary.moodBreakdown.tough) > 0 && (
            <div className="rounded-2xl bg-card border border-border/60 p-4 flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                {summary.moodBreakdown.great > 0 && (
                  <div className="flex items-center gap-1.5">
                    <MoodPill mood="great" />
                    <span className="text-sm text-muted-foreground font-medium">×{summary.moodBreakdown.great}</span>
                  </div>
                )}
                {summary.moodBreakdown.okay > 0 && (
                  <div className="flex items-center gap-1.5">
                    <MoodPill mood="okay" />
                    <span className="text-sm text-muted-foreground font-medium">×{summary.moodBreakdown.okay}</span>
                  </div>
                )}
                {summary.moodBreakdown.tough > 0 && (
                  <div className="flex items-center gap-1.5">
                    <MoodPill mood="tough" />
                    <span className="text-sm text-muted-foreground font-medium">×{summary.moodBreakdown.tough}</span>
                  </div>
                )}
              </div>
              ...
              {(() => {
                ...
                return total > 0 ? (
                  <div className="flex rounded-full overflow-hidden h-2 gap-0.5">
                    {summary.moodBreakdown.great > 0 && (
                      <div
                        className="bg-amber-400/70 dark:bg-amber-500/50"
                        style={{ width: `${(summary.moodBreakdown.great / total) * 100}%` }}
                      />
                    )}
                    {summary.moodBreakdown.okay > 0 && (
                      <div
                        className="bg-sage-400/70 bg-[#86A884]/60"
                        style={{ width: `${(summary.moodBreakdown.okay / total) * 100}%` }}
                      />
                    )}
                    {summary.moodBreakdown.tough > 0 && (
                      <div
                        className="bg-rose-300/70 dark:bg-rose-500/40"
                        style={{ width: `${(summary.moodBreakdown.tough / total) * 100}%` }}
                      />
                    )}
                  </div>
                ) : null;
            ...
            <div className="rounded-2xl bg-amber-50/60 dark:bg-amber-950/20 border border-amber-200/50 dark:border-amber-800/20 p-4 flex flex-col gap-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {summary.allGratitudeItems.map((item, i) => (
                  <span
                    key={i}
                    className="text-xs px-2.5 py-1 rounded-full bg-amber-100/80 dark:bg-amber-900/20 text-amber-800/80 dark:text-amber-300/70 border border-amber-200/40 dark:border-amber-800/20"
                  >
                    {item}
                  </span>
                ))}
